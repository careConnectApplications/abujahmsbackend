...
import User, { usersinterface } from "../models/users";
...
export const updateUserSpecialPermissionService = async (id: mongoose.Types.ObjectId, body: any) => {
  const updatedUser = await User.findByIdAndUpdate(
    id,
    { specialPermissions: body },
    { new: true }
  );

  if (!updatedUser) {
    throw new ApiError(400, configuration.error.errorinvalidcredentials);
  }
...
): Promise<any | null> => User.findById(id);