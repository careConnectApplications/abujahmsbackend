export const mortalityreports = (startdate: any, enddate: any) => {
  // Helper function to create age group pipeline
  const createAgeGroupPipeline = (ageConditions: any) => {
    return [
      {
        $match: {
  ...
  const createMaternalAgeGroupPipeline = (ageConditions: any) => {
    return [
      {
        $match: {
          createdAt: { $gte: startdate, $lte: enddate },
  ...
  const createNeonatalDeathCausePipeline = (cause: string) => {
    return [
      {
        $match: {
          createdAt: { $gte: startdate, $lte: enddate },
  ...
  const createUnderFiveDeathCausePipeline = (cause: string) => {
    return [
      {
        $match: {
          createdAt: { $gte: startdate, $lte: enddate },
